      ...
      try {
        ...
        if (linkMatch) {
          ...
          if (timeDiffInMins <= 2) {
            ...
            const callType = callUrl.includes("/w/vvc.php") ? "voice" : "video";
    ...
    const connectSocket = async () => {
      try {
        ...
        socket.on("callInvitation", async (data) => {
          ...
          if (data.callerId === loggedInUserId) return;

          // Store for persistence
          await Promise.all([
            AsyncStorage.setItem("callUrl", data.callUrl),
            AsyncStorage.setItem("partnerId", data.callerId),
            AsyncStorage.setItem("partnerName", data.callerName || "Someone"),
          ]);
