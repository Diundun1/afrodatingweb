// service-worker.js
const CACHE_NAME = "Diundun-cache-v3";
...
self.addEventListener("push", async (event) => {
  ...
  event.waitUntil(
    (async () => {
      ...
      // ⚡ BROADCAST TO ACTIVE WINDOWS (for immediate popup)
      if (isCall) {
        ...
        for (const client of clientsList) {
          client.postMessage({
            ...
            payload: {
              callUrl: callUrl,
              callerId: data.data?.callerId || data.data?.senderId,
              callerName: senderName,
              room: data.data?.room || data.data?.roomId,
...
self.addEventListener("notificationclick", (event) => {
  ...
  const isAnswering = data.isCall || data.type === "incoming_call" || action === "answer";
            ...
            if (data.isCall || data.type === "incoming_call" || action === "answer") {
              return client.postMessage({
                ...
                payload: {
                  ...
                  autoAnswer: action === "answer", // ⚡ Tell the screen to auto-answer if they clicked the button
                },
              });
            } else {
              return client.postMessage({
