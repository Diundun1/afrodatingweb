...
export default function App() {
  ...
  useEffect(() => {
    ...
    const handler = (event) => {
      ...
      // 🔹 Incoming call
      if (
        (type === "NAVIGATE_TO_CALL" || type === "INCOMING_CALL") &&
        payload?.callUrl
      ) {
        console.log("Navigating to IncomingCallScreen...");
        if (navigationRef.current?.isReady?.()) {
          navigationRef.current.navigate("IncomingCallScreen", {
            callerName: payload.callerName,
            callUrl: payload.callUrl,
            callerId: payload.callerId || payload.partnerId,
            room: payload.room,
            callType: payload.callType || "video",
          });
        } else {
          console.warn("⚠ Navigation not ready for call");
          // Fallback if navigation is not ready
